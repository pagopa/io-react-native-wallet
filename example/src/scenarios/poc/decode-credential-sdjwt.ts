...
import { assert, assertEquals, error, result } from "../types";
...
export default async () => {
  try {
    const { sdJwt, disclosures } = SdJwt.decode(credential);

    // quick check on properties
    assert(sdJwt.payload.verified_claims.claims._sd, Array.isArray);
    assertEquals(disclosures[1]?.decoded[1], "given_name");
    assertEquals(disclosures[1]?.decoded[2] as string, "Mario");

    return result({ sdJwt, disclosures });
  } catch (e) {
    return error(e);
  }
};
